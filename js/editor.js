...
export function renderRaceTab() {
  const box = document.getElementById('raceBody'); if (!box) return;
  const race = S.race, D = S.D;
  const drop = '<div class="drop" id="dropZone"><div>' + t('rcDrop') + ' <label class="browse">' + t('rcBrowse') + '<input type="file" id="fileInp" accept=".gpx,.json,application/gpx+xml,application/json" hidden></label></div><div class="drop-sub">' + t('rcOrJson') + '</div><div id="rcStatus" class="rc-status"></div></div>';
  if (!race) { box.innerHTML = '<div class="panel"><div class="panel-title">' + t('rcTitle') + '</div><div class="nutri-sub">' + t('rcIntro') + '</div>' + drop + '</div>'; wire(box); return; }
  ...
  h += '<div class="rc-actions"><button class="actbtn" id="rcExport">' + t('rcExport') + '</button><button class="actbtn" id="rcImportBtn">' + t('rcImport') + '</button><button class="actbtn danger" id="rcReset">' + t('rcReset') + '</button></div>';
  h += '<div id="rcDropWrap" style="display:none;margin-top:14px">' + drop + '</div></div>';
  // detection
  if (D.hasEle) {
  ...
  if (!wps.length) h += '<div class="nutri-sub">' + t('rcWptNone') + '</div>';
  h += '<div class="ed-list">' + wps.map(w => '<div class="ed-wp k-' + w.kind + '" data-id="' + w.id + '">' +
    ...
    '<button data-a="del" class="ed-del" title="' + t('del') + '">✕</button></div>').join('') + '</div>';
  h += '<div class="rc-actions"><button class="actbtn" id="wpAdd">' + t('rcWptAdd') + '</button></div></div>';
...
function wire(box) {
  ...
  if (D.hasEle) {
    ...
    q('sgRun').addEventListener('click', () => { const ns = autoSegments(D, S.settings.seg); reattachEdits(race.segments, ns); race.segments = ns; commit(true); renderRaceTab(); });
  }
  box.querySelectorAll('.ed-seg').forEach(row => {
    ...
    row.querySelectorAll('[data-f]').forEach(inp => inp.addEventListener(inp.type === 'text' ? 'input' : 'change', () => {
      ...
      else if (f === 'type') { s.type = inp.value; if (s.type === 'climb') { s.speedKmh = null; } commit(true); renderRaceTab(); return; }
      else s[f] = inp.value;
      commit(f === 'key');
    }));
  ...
  box.querySelectorAll('.ed-wp').forEach(row => {
    ...
    row.querySelectorAll('[data-f]').forEach(inp => inp.addEventListener(inp.type === 'text' ? 'input' : 'change', () => {
      const f = inp.dataset.f;
      if (f === 'km') { w.km = Math.max(0, Math.min(D.totalKm, +inp.value || 0)); commit(); renderRaceTab(); return; }
      if (f === 'kind') { w.kind = inp.value; commit(); renderRaceTab(); return; }
      w[f] = inp.value; commit(f === 'time');
    }));
    row.querySelector('[data-a=del]').addEventListener('click', () => { race.waypoints = race.waypoints.filter(x => x.id !== w.id); commit(); renderRaceTab(); });
  });
  q('wpAdd').addEventListener('click', () => { race.waypoints.push({ id: uid('w'), kind: 'ravito', km: +(D.totalKm / 2).toFixed(1), name: '', desc: '', time: '' }); commit(); renderRaceTab(); const last = box.querySelector('.ed-wp:last-child .ed-name'); if (last) last.focus(); });
}
